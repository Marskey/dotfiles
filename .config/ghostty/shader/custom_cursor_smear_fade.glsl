...
void processEdge(vec2 p, vec2 a, vec2 b, inout float minDist, inout float inside) {
    vec2 edge = b - a;
    vec2 pa = p - a;
    // Avoid NaNs when cursor geometry is briefly zero-sized at startup.
    float lenSq = max(dot(edge, edge), 1e-8);
...
const float FOCUS_GUARD_DURATION = 0.05;
const vec3 TRAIL_START_COLOR = vec3(0.0); // Previous cursor: black
const vec3 TRAIL_END_COLOR = vec3(1.0);   // Current cursor: white
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    float baseProgress = clamp(elapsed / DURATION, 0.0, 1.0);

    // Skip stale or partially initialized cursor data. The startup guard
    // prevents a full-screen flash without suppressing legitimate long trails.
    if (iFocus == 0 ||
        iTime - iTimeFocus < FOCUS_GUARD_DURATION ||
        iTime < DURATION ||
        elapsed < 0.0 ||
        baseProgress >= 1.0 ||
    ...
    vec2 currentCenterPixels = iCurrentCursor.xy +
        vec2(iCurrentCursor.z * 0.5, -iCurrentCursor.w * 0.5);
    vec2 previousCenterPixels = iPreviousCursor.xy +
        vec2(iPreviousCursor.z * 0.5, -iPreviousCursor.w * 0.5);
    float movementPixels = distance(currentCenterPixels, previousCenterPixels);
    if (movementPixels < 0.5) {
        return;
    }
    ...
    vec3 trailColor = mix(TRAIL_START_COLOR, TRAIL_END_COLOR, fadeFactor);
    float trailOpacity = clamp(alpha, 0.0, 1.0);
    fragColor.rgb = mix(fragColor.rgb, trailColor, trailOpacity);
